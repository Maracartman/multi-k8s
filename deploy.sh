...
docker build -t maracartman/multi-client:latest -t maracartman/multi-client:$CLIENT_VERSION  -f ./client/Dockerfile ./client
docker build -t maracartman/multi-server:latest -t maracartman/multi-server:$SERVER_VERSION  -f ./server/Dockerfile ./server
docker build -t maracartman/multi-worker:latest -t maracartman/multi-worker:$WORKER_VERSION  -f ./worker/Dockerfile ./worker
#docker build -t maracartman/multi-client:$SHA -f ./client/Dockerfile ./client
#docker build -t maracartman/multi-server:$SHA -f ./server/Dockerfile ./server
#docker build -t maracartman/multi-worker:$SHA -f ./worker/Dockerfile ./worker
